 ...
 *  label 转成image
 *
 *  @param fontName 自提文件对应的字符
 *  @param size     自提大小
 *  @param color    新图片的颜色
 *
 */
+(UIImage *)imageWithFont:(const char *)fontName size:(NSInteger)size color:(UIColor*)color{
    UILabel *label = [[UILabel alloc] init];
    label.font = [UIFont systemFontOfSize:size];
    label.text = [NSString stringWithUTF8String:fontName];
    label.textColor = color;
    label.bounds = CGRectMake(0, 0, size, size);
...
-(UIImage*)imageWithNewSize:(CGSize)size{
    ...
    if (scale == 1) {
        ...
        //宽大于高 上下拉伸
        //        newImage =  [self stretchableImageWithLeftCapWidth:0 topCapHeight:(scale-1)*self.size.height];
        newImage =  [self stretchableImageWithLeftCapWidth:100 topCapHeight:10];
        //        CGFloat top = 25; // 顶端盖高度
        //        CGFloat bottom = 25 ; // 底端盖高度
        //        CGFloat left = 10; // 左端盖宽度
        //        CGFloat right = 10; // 右端盖宽度
        //        UIEdgeInsets insets = UIEdgeInsetsMake(top, left, bottom, right);
        ////        newImage = [self resizableImageWithCapInsets:UIEdgeInsetsMake(0, self.size.height * (scale-1)/2, 0, self.size.height * (scale-1)/2)];
        //        newImage  = [self resizableImageWithCapInsets:insets];
    }else {
        //高大于宽 左右拉伸
        //        newImage = []
        newImage = [self resizableImageWithCapInsets:UIEdgeInsetsMake(0, 0, 0, 0)];
    }
